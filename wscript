...
def configure(ctx):
    ...
    ctx.env.append_unique("LINKFLAGS", ctx.options.linkflags.split())
    print_available(ctx)
...
def options(opt):
    ...
    opt.compiler.add_option("--linkflags",
        action="store",
        type="string",
        default="",
        help="C++ link flags (space separated list)")
