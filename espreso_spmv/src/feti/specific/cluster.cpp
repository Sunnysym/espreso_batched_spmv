...
void ClusterBase::multKplusGlobal_l(SEQ_VECTOR<SEQ_VECTOR<double> > & x_in) {
	...
	if (defprint) { cluster_time.totalTime.start(); }

	if (defprint) { vec_fill_time.start(); }
	fill(vec_g0.begin(), vec_g0.end(), 0); // reset entire vector to 0
	//fill(vec_e0.begin(), vec_e0.end(), 0); // reset entire vector to 0
	if (defprint) { vec_fill_time.end(); }

	// loop over domains in the cluster
	if (defprint) { loop_1_1_time.start(); }
	#pragma omp parallel for
	for (size_t d = 0; d < domains.size(); d++)
	{
	...
	if (defprint) { loop_1_1_time.end();}

	if (defprint) { loop_1_2_time.start(); }
	#pragma omp parallel for
	for (size_t d = 0; d < domains.size(); d++)
	{
...
//	printf (       "Test probe 3: %d norm = %1.30f \n", i, tm1[0][i] );
	...
	if (defprint) { clus_Sa_time.start(); }
//#ifdef SPARSE_SA
...
//		printf (       "Test probe 5: %d norm = %1.30f \n", i, tm1[0][i] );
	...
	if (defprint) { clus_F0_2_time.start(); }
	F0.Solve(tm1[0], vec_lambda,0,0);
	if (defprint) { clus_F0_2_time.end(); }

	if (defprint) { clusCP_time.end(); }
...
//	printf (       "Test probe 6: %d norm = %1.30f \n", i, vec_lambda[i] );
	...
	// Kplus_x
	mkl_set_num_threads(1);
	if (defprint) { loop_2_1_time.start(); }
	#pragma omp parallel for
	for (size_t d = 0; d < domains.size(); d++)
	{
	...
	////ESINFO(PROGRESS3);
	if (defprint) { loop_2_1_time.end(); }

	if (defprint) { cluster_time.totalTime.end(); }
}
...
void ClusterBase::multKplusGlobal_Kinv_opt2(TimeEval& time_eval, SEQ_VECTOR<SEQ_VECTOR<double> > & x_in ) {
	time_eval.totalTime.startWithoutBarrier();
	mkl_set_num_threads(1);

	fill(vec_g0.begin(), vec_g0.end(), 0); // reset entire vector to 0

	time_eval.timeEvents[0].startWithoutBarrier();
	#pragma omp parallel for
	for (size_t d = 0; d < domains.size(); d++)
	{
   		domains[d].B0Kplus_comp.DenseMatVec(x_in[d], tm2[d]);			// g0 - with comp B0Kplus
	...
	time_eval.timeEvents[0].endWithoutBarrier();
	...
	time_eval.timeEvents[1].startWithoutBarrier();
	F0.Solve(vec_g0, tm1[0], 0, 0);
	time_eval.timeEvents[1].endWithoutBarrier();

	time_eval.timeEvents[2].startWithoutBarrier();
	G0.MatVec(tm1[0], tm2[0], 'N');
	time_eval.timeEvents[2].endWithoutBarrier();
	...
	time_eval.timeEvents[3].startWithoutBarrier();
	switch (configuration.SAsolver) {
	case FETIConfiguration::SASOLVER::CPU_SPARSE:
		Sa.Solve(tm2[0], vec_alfa, 0, 0);
		break;
	...
	time_eval.timeEvents[3].endWithoutBarrier();

	time_eval.timeEvents[4].startWithoutBarrier();
	G0.MatVec(vec_alfa, tm1[0], 'T'); 	// lambda
	time_eval.timeEvents[4].endWithoutBarrier();
	...
	time_eval.timeEvents[5].startWithoutBarrier();
	F0.Solve(tm1[0], vec_lambda,0,0);
	time_eval.timeEvents[5].endWithoutBarrier();

	time_eval.timeEvents[6].startWithoutBarrier();
	// Kplus_x
	mkl_set_num_threads(1);
	#pragma omp parallel for
	for (size_t d = 0; d < domains.size(); d++)
	...
	time_eval.timeEvents[6].endWithoutBarrier();
	time_eval.totalTime.endWithoutBarrier();
}
