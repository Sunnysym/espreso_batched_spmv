...
namespace espreso{
	...
	void saveMat(SuperCluster& cluster, double countMem_sc, double countMem_pre)
	{
		char filename[30];
		...
		sprintf(filename, "./save_mat/%04d", m1);
		FILE *fp;
		fp = fopen(filename, "w+");
		fprintf(fp, "%d\n", cluster.my_lamdas_indices.size());
		...
		for (int d = 0; d < cluster.domains.size(); ++d) {
			...
			int cols = cluster.domains[d]->B1Kplus.cols;
			fprintf(fp, "%d %d\n", rows, cols);
		}
		fprintf(fp, "\n");
		for (int d = 0; d < cluster.domains.size(); ++d) {
			int rows = cluster.x_prim_cluster1[d]->size();
			fprintf(fp, "%d\n", rows);
		}
		fprintf(fp, "\n");
		for (int d = 0; d < cluster.domains.size(); ++d) {
			int rows = cluster.domains[d]->Prec.rows;
			int cols = cluster.domains[d]->Prec.cols;
			fprintf(fp, "%d %d\n", rows, cols);
		}
		fclose(fp);
		
		sprintf(filename, "./save_mat/htfeti-%04d", m1);
		fp = fopen(filename, "w+");
		fprintf(fp, "%d %d\n", cluster.G1_comp.rows, cluster.G1_comp.cols);
		fprintf(fp, "%d %d\n", cluster.GGtinvM.rows, cluster.GGtinvM.cols);
		fprintf(fp, "\n");
		fprintf(fp, "%d\n", cluster.clusters.size());
		for (esint c = 0; c < cluster.clusters.size(); ++c)
		{
			fprintf(fp, "%d %d\n", cluster.clusters[c].F0.rows, cluster.clusters[c].F0.cols); // Solve
			fprintf(fp, "%d %d\n", cluster.clusters[c].G0.rows, cluster.clusters[c].G0.cols); // MatVec
			fprintf(fp, "%d\n", cluster.clusters[c].Sa.m_Kplus_size); // Dense Solve
			fprintf(fp, "%d\n", cluster.clusters[c].domains.size());
			for (size_t d = 0; d < cluster.clusters[c].domains.size(); d++)							  // Dense MatVec
			{
				fprintf(fp, "%d %d\n", cluster.clusters[c].domains[d].B0Kplus_comp.rows, cluster.clusters[c].domains[d].B0Kplus_comp.cols);
			}
			for (size_t d = 0; d < cluster.clusters[c].domains.size(); d++)							  // Dense MatVec
			{
				fprintf(fp, "%d %d\n", cluster.clusters[c].domains[d].Kplus_R.rows, cluster.clusters[c].domains[d].Kplus_R.cols);
			}
		}
		fclose(fp);
...
IterSolverBase::IterSolverBase(FETIConfiguration &configuration):
	...
	timeEvalDdot    ("Ddot time"),
	timeEvalNorm    ("Norm time"),
	timeEvalAppaHTFETI("Appa HTFETI timing")
{
	// Timing objects
	...
	TimeEvent p0("x = G1 * lambda");
	TimeEvent p1("MPI_allgather - collect d_mpi");
	TimeEvent p2("local GGtinvM DenseMatVec d_mpi d_local");
	TimeEvent p3("lambda = G1t * x");
	TimeEvent p4("All_Reduce_lambdas");
	timeEvalProj.addEvent(p0);
	timeEvalProj.addEvent(p1);
	timeEvalProj.addEvent(p2);
	timeEvalProj.addEvent(p3);
	timeEvalProj.addEvent(p4);

	TimeEvent t0("x = (c to d) B1 * lambda to xprim");
	TimeEvent t1("Prec * xprim");
	TimeEvent t2("x = B1 * xprim to lambda (d to c)");
	TimeEvent t3("All_Reduce_lambdas");
	timeEvalPrec.addEvent(t0);
	timeEvalPrec.addEvent(t1);
	timeEvalPrec.addEvent(t2);
	timeEvalPrec.addEvent(t3);

	TimeEvent a0("x = (c to d) B1 * lambda, lambda to xprim");
	TimeEvent a1("B1Kplus * dlambda");
	TimeEvent a2("multKplusGlobal xprim"); // can parallel with B1Kplus
	TimeEvent a3("clambda(d to c): dlambda + xprim to lambda");
	TimeEvent a4("All_Reduce_lambdas");
	timeEvalAppa.addEvent(a0);
	timeEvalAppa.addEvent(a1);
	timeEvalAppa.addEvent(a2);
	timeEvalAppa.addEvent(a3);
	timeEvalAppa.addEvent(a4);

	TimeEvent b0("local vec dot");
	TimeEvent b1("AllReduce_sum");
	timeEvalDdot.addEvent(b0);
	timeEvalDdot.addEvent(b1);
	
	TimeEvent c0("local norm add");
	TimeEvent c1("AllReduce_sum");
	timeEvalNorm.addEvent(c0);
	timeEvalNorm.addEvent(c1);


	TimeEvent j0("Loop 1: B0Kplus-mv, KpluR-mv");
	TimeEvent j1("F0 solve - 1st");
	TimeEvent j2("G0 Mult");
	TimeEvent j3("Sa solve");
	TimeEvent j4("G0t Mult");
	TimeEvent j5("F0 solve - 2st");
	TimeEvent j6("Loop 2: B0Kplus-mv, KpluR-mv");
	timeEvalAppaHTFETI.addEvent(j0);
	timeEvalAppaHTFETI.addEvent(j1);
	timeEvalAppaHTFETI.addEvent(j2);
	timeEvalAppaHTFETI.addEvent(j3);
	timeEvalAppaHTFETI.addEvent(j4);
	timeEvalAppaHTFETI.addEvent(j5);
	timeEvalAppaHTFETI.addEvent(j6);
	    ...
	    SEQ_VECTOR < SEQ_VECTOR <double> > & out_dual_solution_parallel)
{
	...
	if (USE_PREC == FETIConfiguration::PRECONDITIONER::DIRICHLET && USE_KINV)
	{
		...
		if (info::ecf->input.save_mat) saveMat(cluster, countMem_sc, countMem_pre);
	    ...
	    SEQ_VECTOR < SEQ_VECTOR <double> > & in_right_hand_side_primal)
{
	...
	apply_A_l_comp_dom_B_opt2(timeEvalAppa, timeEvalAppaHTFETI, cluster, x_l, Ax_l);// apply_A_l_compB(timeEvalAppa, cluster, x_l, Ax_l);
...
for (size_t i = 0; i < y_l.size(); i++)
			...
			ddot_beta.start();
			beta_l =          parallel_ddot_compressed_opt2(timeEvalDdot, cluster, y_l, w_l);
			beta_l = beta_l / parallel_ddot_compressed_opt2(timeEvalDdot, cluster, yp_l, wp_l);
...
for (size_t i = 0; i < p_l.size(); i++)
		...
		//------------------------------------------
		 appA_time.start();
		apply_A_l_comp_dom_B_opt2(timeEvalAppa, timeEvalAppaHTFETI, cluster, p_l, Ap_l); // apply_A_l_compB(timeEvalAppa, cluster, p_l, Ap_l);
		 appA_time.end();

		//------------------------------------------
		 ddot_alpha.start();
		alpha_l =           parallel_ddot_compressed_opt2(timeEvalDdot, cluster, y_l, w_l);
		alpha_l = alpha_l / parallel_ddot_compressed_opt2(timeEvalDdot, cluster, p_l, Ap_l);
		// if (info::ecf->input.use_ddot_opt == 1) {
		...
		#pragma omp parallel for
		 ...
		 norm_time.start();
		norm_l = parallel_norm_compressed_opt2(timeEvalNorm, cluster, w_l);
		 norm_time.end();
...
//			<< indent << std::fixed << std::setprecision(5) << timing.totalTime.getLastStat();
	...
	{
		for (int i = 0; i < timeEvalProj.timeEvents.size(); ++i)
		{
			timeEvalProj.timeEvents[i].eventCount--;
			timeEvalProj.timeEvents[i].eventTime.erase(timeEvalProj.timeEvents[i].eventTime.begin());
			timeEvalProj.timeEvents[i].evaluate();
			info::mpi::printStatsMPI(timeEvalProj.timeEvents[i].eventName, timeEvalProj.timeEvents[i].sumTime);
		}
		timeEvalProj.totalTime.evaluate();
		info::mpi::printStatsMPI("####### timeEvalProj", timeEvalProj.totalTime.sumTime);
		for (int i = 0; i < timeEvalPrec.timeEvents.size(); ++i)
		{
			timeEvalPrec.timeEvents[i].eventCount--;
			timeEvalPrec.timeEvents[i].eventTime.erase(timeEvalPrec.timeEvents[i].eventTime.begin());
			timeEvalPrec.timeEvents[i].evaluate();
			info::mpi::printStatsMPI(timeEvalPrec.timeEvents[i].eventName, timeEvalPrec.timeEvents[i].sumTime);
		}
		timeEvalPrec.totalTime.evaluate();
		info::mpi::printStatsMPI("####### timeEvalPrec", timeEvalPrec.totalTime.sumTime);
		for (int i = 0; i < timeEvalAppa.timeEvents.size(); ++i)
		{
			timeEvalAppa.timeEvents[i].eventCount--;
			timeEvalAppa.timeEvents[i].eventTime.erase(timeEvalAppa.timeEvents[i].eventTime.begin());
			timeEvalAppa.timeEvents[i].evaluate();
			info::mpi::printStatsMPI(timeEvalAppa.timeEvents[i].eventName, timeEvalAppa.timeEvents[i].sumTime);
		}
		timeEvalAppa.totalTime.evaluate();
		info::mpi::printStatsMPI("####### timeEvalAppa", timeEvalAppa.totalTime.sumTime);
		for (int i = 0; i < timeEvalDdot.timeEvents.size(); ++i)
		{
			timeEvalDdot.timeEvents[i].eventCount--;
			timeEvalDdot.timeEvents[i].eventTime.erase(timeEvalDdot.timeEvents[i].eventTime.begin());
			timeEvalDdot.timeEvents[i].evaluate();
			info::mpi::printStatsMPI(timeEvalDdot.timeEvents[i].eventName, timeEvalDdot.timeEvents[i].sumTime);
		}
		timeEvalDdot.totalTime.evaluate();
		info::mpi::printStatsMPI("####### timeEvalDdot", timeEvalDdot.totalTime.sumTime);
		for (int i = 0; i < timeEvalNorm.timeEvents.size(); ++i)
		{
			timeEvalNorm.timeEvents[i].eventCount--;
			timeEvalNorm.timeEvents[i].eventTime.erase(timeEvalNorm.timeEvents[i].eventTime.begin());
			timeEvalNorm.timeEvents[i].evaluate();
			info::mpi::printStatsMPI(timeEvalNorm.timeEvents[i].eventName, timeEvalNorm.timeEvents[i].sumTime);
		}
		timeEvalNorm.totalTime.evaluate();
		info::mpi::printStatsMPI("####### timeEvalNorm", timeEvalNorm.totalTime.sumTime);
		for (int i = 0; i < timeEvalAppaHTFETI.timeEvents.size(); ++i)
		{
			timeEvalAppaHTFETI.timeEvents[i].eventCount--;
			timeEvalAppaHTFETI.timeEvents[i].eventTime.erase(timeEvalAppaHTFETI.timeEvents[i].eventTime.begin());
			timeEvalAppaHTFETI.timeEvents[i].evaluate();
			info::mpi::printStatsMPI(timeEvalAppaHTFETI.timeEvents[i].eventName, timeEvalAppaHTFETI.timeEvents[i].sumTime);
		}
		timeEvalAppaHTFETI.totalTime.evaluate();
		info::mpi::printStatsMPI("####### timeEvalAppaHTFETI", timeEvalAppaHTFETI.totalTime.sumTime);
...
void IterSolverBase::apply_A_l_comp_dom_B_opt2   ( TimeEval & time_eval, TimeEval & time_eval2, SuperCluster & cluster, SEQ_VECTOR<double> & x_in, SEQ_VECTOR<double> & y_out)
{
	// implemeted in itersolvercpu, gpu and acc
}
...
double parallel_norm_compressed_opt2(TimeEval& time_eval, SuperCluster & cluster, SEQ_VECTOR<double> & input_vector )
{
	time_eval.totalTime.start();
	double wl = 0; double wg = 0;
	time_eval.timeEvents[0].start();
	#pragma omp parallel for reduction(+:wl)
	for (size_t i = 0; i < cluster.my_lamdas_indices.size(); i++)
		wl = wl + (input_vector[i] * input_vector[i] * cluster.my_lamdas_ddot_filter[i]);
	time_eval.timeEvents[0].end();
...
double parallel_ddot_compressed_opt2(TimeEval& time_eval, SuperCluster & cluster, SEQ_VECTOR<double> & input_vector1, SEQ_VECTOR<double> & input_vector2 )
{
	time_eval.totalTime.start();
	double a1 = 0; double a1g = 0;
