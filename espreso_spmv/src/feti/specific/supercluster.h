...
public:
	SuperClusterBase(const FETIConfiguration &configuration, DataHolder *instance_in):
		...
		timeKplusGlobal("Appa time Kplus Global")
	{
		if (instance_in != NULL) {
			init();
		}

		TimeEvent vec_fill_time	("Reseting vec_g0 and vec_e0");
		TimeEvent loop_1_1_time	("Loop 1: Kplus-sv, B0-mv, KpluR-mv");
		TimeEvent loop_1_2_time	("Loop 1: vec_e0 and vec_g0");

		// Cluster CP - F0,GO,Sa,G0t,F0;
		TimeEvent clus_F0_1_time	("F0 solve - 1st ");
		TimeEvent clus_G0_time	("G0  Mult ");
		TimeEvent clus_Sa_time	("Sa solve ");
		TimeEvent clus_G0t_time	("G0t Mult ");
		TimeEvent clus_F0_2_time	("F0 solve - 2nd ");


		TimeEvent loop_2_1_time	("Loop2: Kplus-sv, B0-mv, Kplus-mv");

		timeKplusGlobal.addEvent(vec_fill_time);
		timeKplusGlobal.addEvent(loop_1_1_time);
		timeKplusGlobal.addEvent(loop_1_2_time);
		timeKplusGlobal.addEvent(clus_F0_1_time);
		timeKplusGlobal.addEvent(clus_G0_time);
		timeKplusGlobal.addEvent(clus_Sa_time);
		timeKplusGlobal.addEvent(clus_G0t_time);
		timeKplusGlobal.addEvent(clus_F0_2_time);
...
//		ESLOG(MEMORY) << "Total used RAM " << Measure::usedRAM() << "/" << Measure::availableRAM() << " [MB]";
	...
	void multKplusGlobal_Kinv_opt2 (TimeEval& time_eval, SEQ_VECTOR<SEQ_VECTOR<double> *> & x_in) {
		SEQ_VECTOR<SEQ_VECTOR<double> > x_prim_cluster;
		x_prim_cluster.resize(number_of_subdomains_per_supercluster);
		...
		for (esint c = 0; c < numClusters; c++) {
			...
			clusters[c].multKplusGlobal_Kinv_opt2(time_eval, x_prim_cluster );
