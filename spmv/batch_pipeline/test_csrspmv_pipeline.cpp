...
int main(int argc, char* argv[])
{
    ...
    for (int diffmtxnum = 0; diffmtxnum < Diffmtxnum; diffmtxnum += 1)
    {
        ...
        for (int deviceId = 0; deviceId < DeviceNum; ++deviceId)
        {
            ...
            for (int i = 0; i < batchNum; ++i)
            {
                ...
                for(int j = 0; (j < batchMatNum) && ((i * batchMatNum + j) < gpuMatNum) ; j++){
                    CSRMatrix<double>* M = &matrices[gpuMatIdOffset + i * batchMatNum + j];
                    cols += M->cols;
                    nnz += M->nnz;
                    rows += M->rows;
                }
        ...
        for (int iter = 0; iter < IterMax; ++iter)
        {
            ...
            for (int deviceId = 0; deviceId < DeviceNum; ++deviceId)
            {
                ...
                for (int matId = 0; matId < batchNum; ++matId)
                {
                    ...
                    CSRMatrix<double>* M = mat.M;
                    size_t XSiz = M->cols * sizeof(double);
                    HIP_CHECK(hipMemcpyAsync(mat.x_d, M->x_h, XSiz, hipMemcpyHostToDevice, arg.streams[streamId]));
                    ...
                    size_t YSiz = M->rows * sizeof(double);
                    HIP_CHECK(hipMemcpyAsync(M->y_h, mat.y_d, YSiz, hipMemcpyDeviceToHost, arg.streams[streamId]));
                }
                HIP_CHECK(hipDeviceSynchronize());
            ...
            double tsum = std::accumulate(alltimes.begin(), alltimes.end(), 0.);
        ...
        double time_sum = std::accumulate(atimes.begin() + 2,  atimes.end(), 0.);
        double time_avg = time_sum / (IterMax - 2);
        double throughput = 8 * (rownum + colnum) * 1.;
        throughput = throughput / (1024 * 1024 * 1024 * time_avg);
        printf("%.10f\n%.6f\n", time_avg, throughput);
